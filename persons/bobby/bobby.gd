...
@export var character_name: String = "Bobby"
...
func choose_dialogue_entry(entries: Dictionary, options: Dictionary) -> Dictionary:
  ...
  var met: bool = Investigation.get_value("bobby_met")
...
func on_dialogue_node_entered(entry: Dictionary) -> void:
  ...
  print("[Bobby] Dialogue node entered: %s with event_id: %s" % [node_id, event_id])

  if event_id == "":
    return  # No event to process

  if event_id == "first_meeting":
    print("[Bobby] First meeting event triggered on node %s" % node_id)
    Investigation.set_value("bobby_met", true)

  if event_id == "exit_game":
    print("[Bobby] Exit game event triggered on node %s" % node_id)
    get_tree().quit()
